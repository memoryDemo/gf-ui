                  ...
                  <template #default="scope">
                     ...
                     <el-tooltip content="分配角色" placement="top" v-if="scope.row.userId !== 1">
                        <el-button
                           ...
                           v-hasPermi="['system/user/userSelectRole']"
                        ></el-button>
                     </el-tooltip>
                  </template>
               </el-table-column>
...
const { initPassword } = proxy.useConfigValue("sys.user.initPassword");
...
function handleUpdate(row) {
  ...
  getUser(userId).then(response => {
   ...
   //  form.value.postIds = response.postIds;
    ...
    // form.value.roleIds = response.data.user.roleIds;
    open.value = true;
    title.value = "修改用户";
    form.password = "";
  });
...
function usersDept() {
  listDept().then(res => {
    ...
    userList.value.forEach(function(d,i){
      for (let b of res.data) {
        if (d.deptId == b.deptId) {   
          userList.value[i].dept = b;
        };
      };
    })
  });
